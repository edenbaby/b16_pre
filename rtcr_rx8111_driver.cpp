...
LONGWORD CalcRTCR_CompileBCDToSecondsDec(BYTE* LaRTCR_t_Time_Data_Array)
{
	...
	BYTE VeRTCR_y_Year_Count = CalcRTCR_BCDToDec(LaRTCR_t_Time_Data_Array[6]) - 2020; //current year - 2020 = years passed
...
void WriteRTCR_RX8111CE_Register(void)
{
	...
	case RX8111CE_REG_Year:
		...
		break;
	default:
		VsRTCR_h_RX8111CE.register_value = -1;
	}
#endif
...
void displayChipData(void) {
	...
	std::cout << static_cast<int>(VsRTCR_h_RX8111CE.i2c_address) << std::endl;
	std::cout << VsRTCR_h_RX8111CE.register_address << std::endl;
	std::cout << static_cast<int>(VsRTCR_h_RX8111CE.register_value) << std::endl;
	for (int idx = 0; idx < 7; idx++) {
		std::cout << static_cast<int>(VsRTCR_h_RX8111CE.register_expected_values[idx]) << std::endl;
	}
}
#endif
