...
// Define I2C bus and address macros for physical environment
#define I2C_BUS &hi2c1
#define I2C_READ_ADDR(device_addr)  ((device_addr << 1) | 0x01)
#define I2C_WRITE_ADDR(device_addr) ((device_addr << 1) & 0xFE)
...
HAL_StatusTypeDef WriteI2CR_RegisterAddress(uint16_t device_address, uint16_t register_address)
{
    ...
    result = HAL_I2C_Master_Transmit(I2C_BUS, I2C_WRITE_ADDR(device_address), data, size, HAL_MAX_DELAY);
...
HAL_StatusTypeDef ReadI2CR_FromRegister(uint16_t device_address, uint16_t register_address, uint8_t* rx_data, uint16_t size)
{
    ...
    result = HAL_I2C_Master_Receive(I2C_BUS, I2C_READ_ADDR(device_address), rx_data, size, HAL_MAX_DELAY);

    return result;
}

#endif /* PHYSICAL_ENVIRONMENT */